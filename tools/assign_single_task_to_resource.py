...
def assign_project(dfp, dfd, dfr, project_id):
    ...
    if solver.lp[solver.lp.project_id.isin(solver.ld[solver.ld.project_id == project_id].predecessor_id) & solver.lp.finish.isnull()].shape[0]:
        print("predecessor is not assigned for project:", project_id, "(list:", solver.lp[solver.lp.project_id.isin(solver.ld[solver.ld.project_id == project_id].predecessor_id) & solver.lp.finish.isnull()].project_id.to_list(), ")")
        return solver

    if solver.ld[solver.ld.type != 'FS'].shape[0] != 0:
        print("Project doesn't serve other than FS dependencies:", project_id)
        return solver
...
if __name__ == "__main__":
    ...
    dfp.loc[dfp.project_id == project_id, 'worktime'] = pd.Timedelta('56h')
